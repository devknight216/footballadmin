...
export class ActionsArticleComponent implements OnInit {
  ...
  StatusData: any = [];
    ...
    articleKeyword: '';
    insertArticleItem: { };
    insertArticleItemWithPic: boolean = true;
    exampleData = [
        {
          id: 'basic1',
          text: 'Basic 1'
  ...
  getHashtags() {
    ...
    generateCategories() {
        ...
        let arrayOfValues = ['ახალი ამბები', 'მთავარი ამბები'];
        if(!this.ItemData.title)
          this.ItemData.categories = this.CategoriesData.filter(a => arrayOfValues.includes(a.title));
        // this.ItemData.categories = [
        //   {id: 1, title: 'ახალი ამბები', selected: true},
        //   {id: 98,title: 'მთავარი ამბები', selected: true},
        // ];
    }
